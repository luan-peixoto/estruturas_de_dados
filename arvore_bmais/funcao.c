...
TARVBM* copia(TARVBM* arv, int t) {
  ...
  TARVBM* novo = NULL;
  novo = (TARVBM*) malloc(sizeof(TARVBM));
...
int sucessor(TARVBM* arv, int t, int ch) {
  TARVBM* aux = TARVBM_inicializa();
  aux = TARVBM_busca(arv, ch);
  if (!aux) {
    return -1;
  }
  for (int i = 0; i < aux->nchaves; i++) {
    if (aux->chave[i] == ch && i < aux->nchaves - 1) {
      return aux->chave[i+1];
    }
    else {
      if (aux->prox) {
        return aux->prox->chave[0];
      }
      else {
        return MAX_INT;
      }
...
int retira_pares_aux(TARVBM* arv) {
  ...
  for (int i = 0; i < aux->nchaves; i++) {
    ...
    if(aux->chave[i] % 2 == 0 && arv->folha == 1) return aux->chave[i];
  }
...
TARVBM* retira_pares(TARVBM* arv, int t) {
  ...
  while(c != -1) {
    retira_pares(TARVBM_retira(arv, c, t), t);
